...
export const TechStack = () => {
  ...
  const TechCategory = ({
    ...
    techs
      ...
      <motion.div className="grid grid-cols-3 gap-6" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{
      once: true
    }}>
        {techs.map(tech => <motion.div key={tech.name} className="flex flex-col items-center" variants={itemVariants}>
            <div className="w-16 h-16 flex items-center justify-center mb-2 rounded-lg bg-gray-100 dark:bg-gray-800 p-3">
              <img src={tech.icon} alt={tech.name} className="w-full h-full object-contain filter dark:invert-[0.15]" />
            </div>
            <span className="text-sm">{tech.name}</span>
          </motion.div>)}
      </motion.div>
    </div>;
  return <section id="tech-stack" className="py-20 md:py-32 bg-gray-50 dark:bg-black">
